// ====================================================================
// routes-v2.js — Geofences + Sonomètres + Toggle + Pulse (< 3 km)
// Idempotent (protégé si chargé 2x), sans encodage HTML
// ====================================================================

/* Empêche un second chargement accidentel (double <script>) */
if (window.__ROUTES_V2_LOADED__) {
  console.warn("[routes-v2] déjà chargé — second chargement ignoré");
} else {
  window.__ROUTES_V2_LOADED__ = true;
  window.__ROUTES_V2_VERSION__ = "2026-03-17";
  ...
  (function () {
    ...
    // ======================================================
    // 5) CSS dynamique (protégé contre doublons)
    // ======================================================
    if (!document.getElementById("sensor-style")) {
      ...
      sensorStyleEl.textContent = `
        ...
        .sensor-pulse { animation:pulse 1.2s infinite; }
        @keyframes pulse {
    ...
    // ======================================================
    // 6) Geofences + Watcher dynamique vols → zones
    // ======================================================
    window.setupGeofenceWatcher = function (map, geof) {
      ...
      // 6b) Recadrage sur l'ensemble des zones
      if (layers.length) {
      ...
      // 6c) Ajouter les sonomètres
      createSonometersLayer(map);

      // ======================================================
      // 6d) WATCHER DYNAMIQUE (retourné à main.js)
      // ======================================================
    ...
    }; // fin setupGeofenceWatcher

    // ======================================================
    // 7) Bouton ON/OFF pour les sonomètres
    // ======================================================
    window.toggleSensors = function () {
      const btn = document.getElementById("toggle-sensors");
      const theMap = window.map; // présume que main.js expose window.map
      if (!theMap || !window.__SENSORS_LAYER) return;

      if (theMap.hasLayer(window.__SENSORS_LAYER)) {
        theMap.removeLayer(window.__SENSORS_LAYER);
        if (btn) { btn.textContent = "Sonomètres OFF"; btn.classList.add("off"); }
      } else {
        window.__SENSORS_LAYER.addTo(theMap);
        if (btn) { btn.textContent = "Sonomètres ON"; btn.classList.remove("off"); }
      }
    };
