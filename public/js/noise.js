...
if (!window.__NOISE_ENHANCED__) {
  ...
  function heatBadge(v){
    ...
    return `<span class="heat-badge" style="background:${c}"></span><span class="heat-label">${label}</span>`;
  }
  ...
  function filterByHourSpan(items, startHour, endHour){
    return (items||[]).filter(d=>{
      ...
      if (startHour<endHour) return h>=startHour && h<endHour; // ex: 06-18
      return (h>=startHour)||(h<endHour); // ex: 22-06
    });
  }
  ...
  async function renderNoise(){
    ...
    const byId=new Map();
    last24.forEach(d=>{ if(!byId.has(d.id)) byId.set(d.id,[]); byId.get(d.id).push(d); });

    const metaById=new Map(CONFIG.noiseMonitors.map(m=>[m.id,m]));

    if (list){
      const ordered=[...byId.keys()].sort((a,b)=>{
        const A=metaById.get(a)?.name||a; const B=metaById.get(b)?.name||b;
        return A.localeCompare(B)||a.localeCompare(b);
      });
      ordered.forEach(id=>{
        const data=byId.get(id); const meta=metaById.get(id)||{name:id};
        const latest=data[data.length-1];
        const { Lden, Lnight }=computeLdenLnight(data);
        const heat=heatBadge(Lden);
        const li=document.createElement('li'); li.className='noise-item';
        li.innerHTML=`
          <div class="noise-row">
      ...
      if (!ordered.length){ const li=document.createElement('li'); li.textContent='Aucune donnée (24h).'; list.appendChild(li); }
    }
    ...
    if (canvas){
      ...
      window.__NOISE_CHART__=new Chart(ctx,{
        type:'line',
        data:{ labels, datasets:[{ label:'dB(A)', data:values, borderColor:'#3a7bd5', backgroundColor:'rgba(58,123,213,0.15)', tension:0.25, pointRadius:0 }] },
